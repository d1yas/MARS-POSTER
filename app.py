...
# TOKEN = "7554360867:AAEbWb9TRsredE2JYZShuqpu7xARCOIrHP8"
# ADMINS = ["6812498519","2122893555"]
...
async def ask_for_post_photo(message: types.Message):
    ...
    await message.answer("📤 Iltimos, post uchun rasm yuboring yoki kanaldan xabarni forward qiling:", reply_markup=btn_cancel)
...
async def join_group(message: types.Message):
    # Admin qo'shgan guruhlarni tekshirish
    bot_info = await bot.get_me()
    
    # Guruh admini kim ekanligini tekshirish
    chat_member = await bot.get_chat_member(message.chat.id, message.from_user.id)
    
    # Agar guruhga botni qo'shgan odam admin bo'lmasa, guruhdan chiqib ketish
    if not is_admin(message.from_user.id) and chat_member.status not in ['creator', 'administrator']:
        # Bot guruhdan chiqib ketadi
    ...
    for new_member in message.new_chat_members:
        if new_member.id == bot_info.id:
            ...
            for admin_id in ADMINS:
                try:
                    await bot.send_message(admin_id, f"✅ Bot yangi guruhga qo'shildi: {group_name} ({group_id})")
                except Exception as e:
                    print(f"Admin {admin_id}ga xabar yuborib bo'lmadi: {e}")
